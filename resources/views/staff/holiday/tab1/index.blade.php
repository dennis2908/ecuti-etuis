...
<div class=" container-fluid container-fixed-lg bg-white">
	...
	<div class="card card-transparent mb-0 mt-3">
		<form class="p-b-10" id="form-general" role="form" autocomplete="off" method="post" action="{{ route('staff.holiday.general') }}" novalidate>
					...
					<div class="col-md-6">	
						<div class="form-group form-group-default form-group-default-custom form-group-default-select2 required">
							...
							<select name="holiday_type_id" id="holiday_type_id" class="full-width autoscroll" data-init-plugin="select2" required="">
								<option selected value="-1">-- Semua Cuti Persekutuan --</option>
			...
			<table class="table table-hover " id="table-general">
				<thead>
					<tr>
						...
						<th class="fit">Status Aproved</th>
						<th class="fit">Aproved Oleh</th>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addModalTitle">Tambah <span class="bold">Cuti Persekutuan</span></h5>
                <small class="text-muted">Sila isi maklumat pada ruangan di bawah.</small>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
			<form id="form-add-general" role="form" method="post" action="{{ route('staff.holiday.general') }}">
...
var settings = {
	...
	"serverSide": true,
	"deferRender": true,
	"ajax": "{{ route('staff.holiday.general') }}",
	"columns": [
...
function editGeneral(id) {
	$("#modal-div").load("{{ route('staff.holiday.general') }}/"+id);
}
...
function removeGeneral(id) {
	...
	.then((confirm) => {
		if (confirm) {
			$.ajax({
				url: '{{ route('staff.holiday.general') }}/'+id,
				method: 'delete',
				dataType: 'json',
				async: true,
				contentType: false,
